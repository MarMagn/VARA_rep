...
getSensSpec <- function(my_data, getModelResults, no_splits = 10) {
  ...
  # if (mean(result_df$AUC > 1)) {browser()}
...
names(tmp) <- c("30 days with index codes", "30 with readmission codes", "30 days",  "90 days with index codes", "90 with readmission codes", "90 days", "90 days with index codes major", "90 with readmission codes major", "90 days major")
htmlTable(
...
htmlTable(
  ...
  rgroup = names(tmp),
  n.rgroup = sapply(tmp, nrow),
  caption = "All patients small",
  tfoot = names(small_datasets$`30_index_c`)
)
```
