...
**Random forest**
```{r}
all_results_30[['Random forest crude ']] <- getSensSpec(major_30, getModelResults = function(train, test) {
...
all_results_90[['Random forest crude ']] <- getSensSpec(major_90, function(train, test) {
  cross_model <- ranger(has_AE~., data = train)
  p <- predict(cross_model, data = test )
  rfRoc <- data.frame(response = test$has_AE, predictor = p$predictions) %>%
    arrange(predictor) %>%
...
all_results_30[['Random forest tweak ']] <- getSensSpec(data_30_n, function(train, test) {
  set.seed(543)
  cross_model <- ranger(has_AE~., data = train, mtry = 11, min.node.size = 1)
  p <- predict(cross_model, data = test )
  rfRoc <- data.frame(response = test$has_AE, predictor = p$predictions) %>%
    arrange(predictor) %>%
    with(., roc(response, predictor))
...
all_results_30[['Logistic regresssion crude']] <- 
  getSensSpec(major_30, function(train, test) {
...
all_results_90[['Logistic regresssion crude']] <- 
  getSensSpec(major_90, function(train, test) {
    cross_model_log <- glm(has_AE ~., 
                           family = binomial(link = logit), 
                           data = train)
    p <- predict(cross_model_log, newdata = test, type = "response")
...
With splines
```{r}
all_results_30[['Logistic regresssion splines crude']] <- getSensSpec(major_30, function(train, test) {
...
all_results_30[['Logistic regresssion splines test']] <- getSensSpec(tester, function(train, test) {
...
all_results_30[['Logistic regresssion splines crude tweak']] <- getSensSpec(tester, function(train, test) {
  set.seed(876)
  cross_model_log <- glm(has_AE ~., 
                           family = binomial(link = logit), 
                           data = train)
...
all_results_30[['Logistic regresssion splines tweak']] <- 
  getSensSpec(data_30_n, function(train, test) {
    set.seed(543)
    cross_model_log <- glm(has_AE ~., 
                           family = binomial(link = logit), 
                           data = train)#, 
    cross_model_log %<>% update(.~.- los + ns(los, 2))
    p <- predict(cross_model_log, newdata = test, type = "response")
    logRoc <- data.frame(response = test$has_AE, predictor = p) %>%
      arrange(predictor) %>%
      with(., roc(response, predictor))
...
**SVM**
```{r}
all_results_30[['SVM crude']] <- getSensSpec(y, function(train, test) {
...
nn_data <- ds
  ...
  nn_data[, 2:3] <- data.frame(lapply(nn_data[,2:3], scl))
  nn_data[, 12:17] <- data.frame(lapply(nn_data[,12:17], scl))
  neural_datasets <- list()
  for (days in c(30, 90)) {
    ds_name <- sprintf("%s", days)
    drop_vars <- c("index_ae", "maj_30", "maj_90", "serial_no")
    if (days == 30) {
    ...
    neural_datasets[[sprintf("%s_wc", ds_name)]] <- select(nn_data, -!!drop_vars, !!rename_vars)
    neural_datasets[[ds_name]] <- neural_datasets[[sprintf("%s_wc", ds_name)]] %>% 
      select(-!!raw_var)
  }
  return(neural_datasets)
}

neural_datasets <- getNeuralDatasets(no_na_data)
```
