...
all_results[['Open comparison 30 days']] <- res_30
all_results[['Open comparison 90 days']] <- res_90

```

*Preparing dataset fot analysis and modelling*
```{r}
...
rm(AE_frame, AEs, ag_data, data_3,found_AEs, index, readm, rrr_all, rrr_data)
```
...
all_results = list()
class(all_results) <- c("results_class", class(all_results))
...
**Random forest**
```{r}
all_results[['Random forest crude 30']] <- getSensSpec(data_30_n, function(train, test) {
...
all_results[['Random forest crude 90']] <- getSensSpec(data_90_n, function(train, test) {
  cross_model <- ranger(has_AE~., data = train)
  p <- predict(cross_model, data = test )
  rangerRoc <- data.frame(response = test$has_AE, predictor = p$predictions) %>%
    arrange(predictor) %>%
...
```{r}
all_results[['Random forest weighted 30']] <- getSensSpec(data_30_n, function(train, test) {
...
all_results[['Random forest weighted 90']] <- getSensSpec(data_90_n, function(train, test) {
  cross_model <- ranger(has_AE~., data = train, case.weights = (1 +(train$has_AE == 0)*3.7))
  p <- predict(cross_model, data = test)
  rangerRoc <- data.frame(response = test$has_AE, predictor = p$predictions) %>%
    arrange(predictor) %>%
...
```{r}
all_results[['Logistic regresssion crude 30']] <- 
...
all_results[['Logistic regresssion crude 90']] <- 
...
all_results[['Logistic regresssion weighted 30']] <- 
  getSensSpec(data_30_n, function(train, test) {
    cross_model_log <- glm(has_AE ~., 
                           family = binomial(link = logit), 
                           data = train, 
...
all_results[['Logistic regresssion weighted 90']] <- 
  getSensSpec(data_90_n, function(train, test) {
    cross_model_log <- glm(has_AE ~., 
                           family = binomial(link = logit), 
                           data = train, 
 ...
 ##with splines??
all_results[['Logistic regresssion weighted - los']] <- 
  getSensSpec(no_na_data_n, function(train, test) {
    cross_model_log <- glm(has_AE ~., 
                           family = binomial(link = logit), 
                           data = train)#, 
                          # weights = (1 +(train$has_AE == "No")*1))
...
**SVM**
```{r}
all_results[['SVM weighted 30']] <- getSensSpec(data_30, function(train, test) {
 ...
 all_results[['SVM weighted 90']] <- getSensSpec(data_90, function(train, test) {
  cross_model <- svm(has_AE~., data = train, class.weights = c(Yes =  1, No = 2))
  p <- predict(cross_model, newdata = test)
  p <- ifelse(p == "Yes", 1, 0)
  svmRoc <- data.frame(response = test$has_AE, predictor = p) %>%
...
sens <- res_tbl['1', '1']/(res_tbl['1', '1']+res_tbl['1', '0'])
spec <- res_tbl['0', '0']/(res_tbl['0', '0']+res_tbl['0', '1'])
